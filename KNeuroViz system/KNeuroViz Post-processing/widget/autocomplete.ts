...
// import { bindDefaultPasteHandler } from '../ui/default_clipboard_handling';
...
export class AutocompleteTextInput extends RefCounted {
  ...
  constructor(options: {completer: Completer, delay?: number}) {
    ...
    // inputElement.readOnly = true; //Joe
    // inputElement.value="precomputed://http://192.168.1.6:8080/src/sample"; //Joe

    this.inputElement.addEventListener("input", function(){ //Joe
      console.log("데이터 url 입력");
      // alert("데이터 url 입력");
  ...
  get disabled() {
    return this.inputElement.readOnly; //이 부분 기존 소스와 비교해 볼 것 Joe
  }

  set disabled(value: boolean) {
    this.inputElement.readOnly = value; //이 부분 기존 소스와 비교해 볼 것 Joe
  }
